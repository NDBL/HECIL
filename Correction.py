...
# version 8 : edits
# decide confidence threshold based on value of J1 (not difference of J1,J2)
# if J1>1.5, then high confidence


# version 7 : edits
# for low-confidence correction, do not replace with reference base (this will not update the corrected long reads after each iteration)
# Instead, substitute with the allele that has highest sum of weights
########################################################

#version 6: iterative correction, cleaner code

########################################################

#version 6 : edits
#iterative correction - in each iteration, mark low-confidence corrections and create a separate pileup file to store those lines.
#Create a wrapper file to call correction algo -> downstream scripts (corrected_allReads) -> VARI (count bubbles) -> based on output/bubble keep iterating

#version 5 : edits
#1. normalize quality score-based weight
#2. for the final corrected read, check for '+\d' (nonATGCN) format and remove those (no need to rrun 'Remove_nonATGCN.py' later)

#============================================================
#version 4 edits: 
#1. A reference header can have multiple range of positions (Eg. 100->200, 350->400). Earlier version considered the leftmost pos as start and rge rightmost pos as end (Eg. 100->400). Hence, the bases in between (Eg. 201->349) were getting deleted in the output file/corrected reads. That is fixed by creating dict_pilepos as list of values, each value in the form 'startposi_endposi'
#2. for else conditions (at the end, while double-checking), added max_base value


#This program corrects erroneous LR bases based on the SR 
#that has highest sum of normalized weights (based on edit distance and mapping quality)
		...
		#---------------------------------------------------------
				...
				# Added in version 6 - check for low-confidence corrections, keep as it is
					...
					if ((al_highest==',' and al_sechighest=='.') or (al_highest=='.' and al_sechighest==',')):
						...
						# Updated in version 8
						if (int(med_highest)<conf_threshold):
							nline=line+'\n'
							f_pileupiter.write(nline)
							#max_base=ref_base
							# Updated in v7 -> Don't replace with ref base
						max_base=al_highest
						#else:
						#	max_base=al_highest
				#print l_SR_sumweight
